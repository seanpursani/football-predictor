...
INSERT INTO cron_config (key, value, description) VALUES
  ...
  (
    ...
    '*/30 * * * *',
    'Cron expression: ingest-events polling (default: every 30 minutes — polls for match events continuously during gameweek)'
  )
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'ingest-odds-weekly') THEN
    PERFORM cron.unschedule('ingest-odds-weekly');
  END IF;

  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'ingest-events-polling') THEN
