...
export async function fetchMatchEvents(
  ...
  if (!apiKey) {
    return {
      data: null,
      error: { code: EVENTS_ERROR_CODES.MISSING_API_KEY, message: 'EVENTS_API_KEY environment variable is not set' },
    };
  }
  ...
  try {
    ...
    return {
      data: null,
      error: { code: EVENTS_ERROR_CODES.INVALID_RESPONSE, message: 'Events API response did not match expected shape' },
    };
  }
}
